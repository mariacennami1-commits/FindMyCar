...
<resources>
    ...
    protected void onStop() {
        ...
        if old_onstop in source:
            ...
            info("fix_black_screen: WARNING - onStop() pattern not found in PythonActivity.java - trying alternative")
            # Try alternative pattern (different p4a version)
            alt_onstop = '''    @Override
    protected void onStop() {
        Log.v("PythonActivity", "onStop()");
        this.mActivity.mOpened = true;
    ...
    protected void onStop() {
        ...
        if (this.mActivity.mLoadFinished) {
            ...
            if alt_onstop in source:
                source = source.replace(alt_onstop, alt_new)
                with open(java_path, "w") as f:
                    f.write(source)
                info("fix_black_screen: Patched PythonActivity.onStop() (alt pattern)")
            else:
                info("fix_black_screen: WARNING - no onStop() pattern matched")
                info("fix_black_screen: PythonActivity.java content:\n" + source[:2000])
    else:
